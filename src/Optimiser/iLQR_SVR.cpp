...
iLQR_SVR::iLQR_SVR(std::shared_ptr<ModelTranslator> _modelTranslator, std::shared_ptr<MuJoCoHelper> _MuJoCo_helper,
                   std::shared_ptr<Differentiator> _differentiator, int horizon, std::shared_ptr<Visualiser> _visualizer,
...
//        std::cout << "setup threads: " << duration_cast<microseconds>(high_resolution_clock::now() - temp_timer).count() / 1000.0f << "ms \n";
    ...
    if(verbose_output){
        PrintBannerIteration(iteration_num, new_cost, old_cost,
                             1 - (new_cost / old_cost), lambda, dof,
...
bool iLQR_SVR::CheckForConvergence(double old_cost, double new_cost){
...
void iLQR_SVR::GenerateDerivatives(){

    // Compute dynamics derivatives at key-points and interpolate the remainder
    auto start_fd_time = high_resolution_clock::now();

    // Hack - from code that use to assume that we used keypoints for dynamics derivatives in
    // previous repository - TODO - fix this.
    keypointsGlobal.clear();
    for(int t = 0; t < horizon_length; t++){
        keypointsGlobal.push_back(std::vector<int>());
        for(int i = 0; i < dof; i++){
            keypointsGlobal[t].push_back(i);
        }
    }
...
void iLQR_SVR::ComputeDynamicsDerivatives(){

    MuJoCo_helper->InitModelForFiniteDifferencing();

    // Debug code, compute derivatives in serial for debugging purposes
...
//    }
    ...
    current_iteration = 0;
    num_threads_iterations = horizon_length;
    ...
    for (int i = 0; i < num_threads; ++i) {
        thread_pool.push_back(std::thread(&iLQR_SVR::WorkerComputeDerivatives, this, i));
    }
...
void iLQR_SVR::ComputeCostDerivatives(){
...
void iLQR_SVR::ComputeResidualDerivatives(){
    ...
    std::vector<std::thread> thread_pool;
    for (int i = 0; i < num_threads; ++i) {
        thread_pool.push_back(std::thread(&iLQR_SVR::WorkerComputeResidualDerivatives, this, i));
    }
...
void iLQR_SVR::WorkerComputeDerivatives(int threadId) {
    while (true) {
        ...
        (activeDifferentiator.get()->*(tasks_dynamics_derivs[iteration]))(A[iteration], B[iteration],
                                                                          keypointsGlobal[iteration],
                                                                          iteration, threadId, true, 1e-6);
    }
}

void iLQR_SVR::WorkerComputeResidualDerivatives(int threadId){
    while (true) {
        int iteration = current_iteration.fetch_add(1);
        if (iteration >= num_threads_iterations) {
            break;  // All iterations done
...
void iLQR_SVR::SaveSystemStateToRolloutData(mjData *d, int thread_id, int data_index){
    ...
    // TODO - should this be data index - 1?
